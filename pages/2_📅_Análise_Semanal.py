...
if st.session_state.error_occurred:
    ...
    page_title = "MAVIS - Análise Semanal"
...
riscos_semana = st.session_state.get('riscos_semana', {})

if df_semanal.empty:
    st.warning("Não há dados semanais para a receita selecionada.")
    st.stop()

df_final = df_semanal.copy()
if filtros.get('anos'): df_final = df_final[df_final['SEMANA'].dt.year.isin(filtros['anos'])]
if filtros.get('meses'): df_final = df_final[df_final['SEMANA'].dt.month.isin(filtros['meses'])]
if filtros.get('semanas'): df_final = df_final[df_final['SEMANA'].dt.isocalendar().week.isin(filtros['semanas'])]

if df_final.empty:
    st.info("Nenhum dado encontrado para os filtros selecionados.")
else:
    df_final['ANO_SEMANA'] = df_final['SEMANA'].dt.strftime('%Y-S%V')
    ...
    # st.markdown(analysis.detectar_anomalia(df_final, sensor_atual))
    # st.line_chart(df_final.set_index('SEMANA')[sensor])

    # with st.expander("Ver análise detalhada da IA"):
    #     st.caption(analysis.analisar_tendencia(df_final, sensor_atual, sensor=sensor_atual))
    #     st.markdown("---")
